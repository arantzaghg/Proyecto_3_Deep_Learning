...
def run_models():
    ...
    train_data, stats = preprocess_data(train_data, ticker, alpha=0.0125 , stage="train", include_close=True)
    test_data, _ = preprocess_data(test_data, ticker, alpha=0.0125 , stage="test", stats=stats, include_close=True)
    val_data, _ = preprocess_data(val_data, ticker, alpha=0.0125 , stage="val", stats=stats, include_close=True)
    ...
    train_signals_cnn(x_train, y_train, x_test, y_test, x_val, y_val, params_cnn, epochs=50, batch_size=32)

    # Train MLP models
    mlflow.set_experiment("MLP model")
    train_signals_mlp(x_train, y_train, x_test, y_test, x_val, y_val, params_mlp, epochs=50, batch_size=32)
